...
class SPDRLHFDataset(RLHFDataset):
    ...
    def __getitem__(self, item):
        ...
        if "input_ids" in row_dict and row_dict["input_ids"] is not None:
            ...
            # Ensure it's a list of ints or convertable to tensor
            ...
            # Assuming all provided tokens are valid valid tokens (no padding in source)
            attention_mask = torch.ones_like(input_ids, dtype=torch.long)
            
            # 3. Apply padding/truncation using verl utility
            # postprocess_data expects input shape of (batch_size, seq_len), so we unsqueeze
            ...
            # 5. Update row_dict with processed tensors
            row_dict["input_ids"] = input_ids
            row_dict["attention_mask"] = attention_mask
            row_dict["position_ids"] = position_ids
            
            # 6. Update extra_info indices for padding
            # Calculate padding length (assuming left padding to max_prompt_length)
            # input_ids is now (max_length,)
            # original length was len(input_ids_list)
            pad_len = self.max_prompt_length - len(input_ids_list)
            ...
            if "extra_info" in row_dict and row_dict["extra_info"] is not None:
                ...
                for key in ["draft_start_idx", "draft_end_idx", "target_start_idx", "target_end_idx"]:
                    if key in row_dict["extra_info"]:
                         row_dict["extra_info"][key] += pad_len
